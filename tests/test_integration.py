import pytest
from unittest.mock import patch, MagicMock
from veildata.pipeline import DetectionPipeline
from veildata.detectors import RegexDetector, SpacyDetector, BertDetector, HybridDetector, EntitySpan
from veildata.revealers import TokenStore


def test_detection_pipeline_with_regex():
    """Test detection pipeline with regex detector."""
    patterns = {"EMAIL": r"[a-z]+@[a-z]+\\.com"}
    detector = RegexDetector(patterns)
    store = TokenStore()
    pipeline = DetectionPipeline(detector, store=store)
    
    text = "Contact test@example.com"
    masked = pipeline(text)
    
    assert "[REDACTED_1]" in masked
    assert "test@example.com" not in masked
    assert store.mappings["[REDACTED_1]"] == "test@example.com"


@patch('veildata.detectors.spacy')
def test_detection_pipeline_with_spacy(mock_spacy):
    """Test detection pipeline with spaCy detector."""
    # Mock spaCy model and doc
    mock_nlp = MagicMock()
    mock_spacy.load.return_value = mock_nlp
    
    # Mock entity
    mock_ent = MagicMock()
    mock_ent.label_ = "PERSON"
    mock_ent.start_char = 0
    mock_ent.end_char = 4
    mock_ent.text = "John"
    
    # Mock doc
    mock_doc = MagicMock()
    mock_doc.ents = [mock_ent]
    mock_nlp.return_value = mock_doc
    
    detector = SpacyDetector(pii_labels=["PERSON"])
    pipeline = DetectionPipeline(detector)
    
    result = pipeline("John works at Apple")
    assert "[REDACTED_1]" in result
    assert "Apple" in result  # Not in our entities list


def test_hybrid_detection_pipeline():
    """Test hybrid detection pipeline with multiple detectors."""
    # Create mock detectors
    regex_detector = RegexDetector({"EMAIL": r"[a-z]+@[a-z]+\\.com"})
    
    # Mock spaCy detector
    mock_nlp = MagicMock()
    with patch('veildata.detectors.spacy.load', return_value=mock_nlp):
        mock_doc = MagicMock()
        mock_ent = MagicMock()
        mock_ent.label_ = "PERSON"
        mock_ent.start_char = 0
        mock_ent.end_char = 4
        mock_ent.text = "John"
        mock_doc.ents = [mock_ent]
        mock_nlp.return_value = mock_doc
        
        spacy_detector = SpacyDetector(pii_labels=["PERSON"])
        
        # Create hybrid detector
        hybrid_detector = HybridDetector(
            [regex_detector, spacy_detector],
            strategy="union",
            prefer="ml"
        )
        
        # Create pipeline
        store = TokenStore()
        pipeline = DetectionPipeline(hybrid_detector, store=store)
        
        # Test with text containing both email and person
        text = "John's email is john@example.com"
        masked = pipeline(text)
        
        # Both should be masked
        assert "[REDACTED_1]" in masked  # John
        assert "[REDACTED_2]" in masked  # email
        assert "john@example.com" not in masked
        
        # Verify store
        assert len(store.mappings) == 2
        assert "John" in store.mappings.values()
        assert "john@example.com" in store.mappings.values()


def test_pipeline_with_custom_mask_format():
    """Test pipeline with custom mask format."""
    detector = RegexDetector({"EMAIL": r"[a-z]+@[a-z]+\\.com"})
    store = TokenStore()
    
    # Custom mask format
    pipeline = DetectionPipeline(
        detector,
        store=store,
        mask_format="[MASKED_{counter}]"
    )
    
    text = "Contact test@example.com"
    masked = pipeline(text)
    
    assert "[MASKED_1]" in masked
    assert store.mappings["[MASKED_1]"] == "test@example.com"
    assert pipeline.unmask(masked) == text
